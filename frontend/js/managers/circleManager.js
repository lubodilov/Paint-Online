function startCircle(c, ev) {
  ...
  c.cur_figure = new Circle(line_conf, color, image_data);
  c.cur_figure.startCircle(ev.offsetX, ev.offsetY);
...
function updateCircle(c, ev) {
  if (!c.cur_figure) return;
  c.ctx.clearRect(0, 0, c.canvas_width, c.canvas_height);
  c.cur_figure.updateCircle(ev.offsetX, ev.offsetY);
...
function finishCircle(c, ev) {
  if (!c.cur_figure) return;
  c.cur_figure.finishCircle();
